...
public class StopAndWaitReceiver {
     private final int port;
      ...
      public StopAndWaitReceiver(int port) {
        this.port = port;
    }
    ...
    public void receive(String outputPath) throws IOException {
        ...
        while (true) {
            ...
            if (packet.hasFlag(Packet.FLAG_FIN)) {
                System.out.println("FIN received — saving file...");
            ...
            if (seq == expectedSeq) {
                ...
                expectedSeq++;
                System.out.printf("  ← SEQ=%d (%d bytes) ✓%n", seq,
                    packet.payload != null ? packet.payload.length : 0);
            } else {
                System.out.printf("  ← SEQ=%d (expected %d) — duplicate%n", seq, expectedSeq);
            }
     ...
     private void sendAck(DatagramSocket socket, InetAddress addr, int port, int ackNumber)
            throws IOException {
    ...
    private void sendFinAck(DatagramSocket socket, InetAddress addr, int port, int ackNumber)
            throws IOException {
    ...
    private void writeFile(TreeMap<Integer, byte[]> chunks, String outputPath)
            throws IOException {
